...
def VelVer(dat) : # R, P, qF, qB, pF, pB, dtI, dtE, F1, Hij,M=1): # Ionic position, ionic velocity, etc.
    ...
    for t in range(int(np.floor(EStep/2))):
        qF, pF = Umap(qF, pF, par.dtE, dat.Hij)
...
def pop(dat):
    if dat.param.stype == 'square':
        ...
        # Taken (Stolen) from Braden's (Braden Weight) Implementation
        # Check his Package : https://github.com/bradenmweight/QuantumDynamicsMethodsSuite
        for i in range(N):
            for j in range(N):
                if (i == j):
                    if ( η[j] - 1 < 0.0 or η[j] - 1 > 2 * γ ):
                        ρij[i,i] = 0
                if (i != j):
                    if ( η[j] < 0.0 or η[j] > 2 * γ ):
                        ρij[i,i] = 0
